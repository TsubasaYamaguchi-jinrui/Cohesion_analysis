...
作成したデータは以下の通り。  
```{r}
datatable(focal_combined_prox_final,
...
## 切断モデル  
m_cohesion_an_trunc <- brm(log(mean_RG)|trunc(lb = 0) ~ TY_presence + IT_presence + ntm_std + est_std + 
                       female_std + kin_std  + study_period  + (1|subject),
                                 family = student,
...
## 切断モデル  
m_cohesion_an_bf21_trunc <- brm(log(mean_RG)|trunc(lb = 0) ~ TY_presence + IT_presence + ntm_std + est_std + 
                                  female_std + kin_std  + study_period + (1|subject),
                                family = student,
                                iter = 11000, warmup = 1000, seed = 123,
                                prior = c(prior(student_t(4,0,5), class = "b"),
...
model_parameters(m_cohesion_an,  ci = 0.95,
...
model_parameters(m_cohesion_an_bf21,  ci = 0.95,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,3,5,6, 7,8) %>% 
...
```{r}
cohesion_data_es <- focal_combined_prox_final %>% 
  mutate(rate_female = no_female/max_female) %>%
  filter(rs2 == 1) %>% 
  replace_na(list(hud = 0)) %>% 
  filter(hud != "1" & consort == 0) %>% 
  filter(RG == 1 & T_G == "G") %>% 
  group_by(date, no_focal, subject, study_period, rate_agg_6yo, no_ntm, 
           no_est, no_male, no_female, TY_presence,
           no_kin, IT_presence, CSI_TY) %>% 
...
```{r}
## 2019~2021年
m_cohesion_es <- brm(log(mean_RG) ~ TY_presence + IT_presence + ntm_std + est_std + 
                       female_std + kin_std  + study_period + (1|subject),
                                 family = student,
                                 iter = 11000, warmup = 1000, seed = 123,
                                 prior = c(prior(student_t(4,0,5), class = "b"),
                                 ...
                                 control = list(adapt_delta = 0.99, max_treedepth = 20),
                                 backend = "cmdstanr",
                                 data = cohesion_data_es,
                                 file = "model/m_cohesion_es.rds")

## 切断モデル  
m_cohesion_es_trunc <- brm(log(mean_RG)|trunc(lb = 0) ~ TY_presence + IT_presence + ntm_std + est_std + 
                       female_std + kin_std  + study_period  + (1|subject),
                                 family = student,
                                 iter = 7000, warmup = 2000, seed = 123,
                                 prior = c(prior(student_t(4,0,5), class = "b"),
...
## 2019~2020年
cohesion_data_es_bf21 <- cohesion_data_es %>% 
  filter(study_period != "m21") %>% 
  mutate(ntm_std = standardize(no_ntm),
         male_std = standardize(no_male),
         est_std = standardize(no_est),
...
m_cohesion_es_bf21 <- brm(log(mean_RG) ~ TY_presence + IT_presence + ntm_std + est_std + 
                       female_std + kin_std  + study_period + (1|subject),
                                 family = student,
                                 iter = 11000, warmup = 1000, seed = 123,
                                 prior = c(prior(student_t(4,0,5), class = "b"),
                                 ...
                                 control = list(adapt_delta = 0.99, max_treedepth = 20),
                                 backend = "cmdstanr",
                                 data = cohesion_data_es_bf21,
                                 file = "model/m_cohesion_es_bf21.rds")

## 切断モデル  
m_cohesion_es_bf21_trunc <- brm(log(mean_RG)|trunc(lb = 0) ~ TY_presence + IT_presence + ntm_std + est_std + 
                                  female_std + kin_std  + study_period + (1|subject),
                                family = student,
                                iter = 11000, warmup = 1000, seed = 123,
                                prior = c(prior(student_t(4,0,5), class = "b"),
...
#### モデルチェック  
まず、DHARMaパッケージ[@Hartig2022]とDHARMa.helperパッケージ[@Francisco2023]でモデルの前提が満たされているかを確認します。あまり当てはまりが良くないよう。  
```{r, fig.height = 4}
## 全期間    
dh_cohesion_es <- dh_check_brms(m_cohesion_es, quantreg = TRUE)

## 21年より前
dh_cohesion_es_bf21 <- dh_check_brms(m_cohesion_es_bf21, quantreg = TRUE)
```
<br/>  
...
```{r}
## 全期間   
check_collinearity(m_cohesion_es)

## 21年より前
check_collinearity(m_cohesion_es_bf21)
```

bayesplotパッケージ[@Gabry2022]の`pp_check`関数で、事後分布からの予測分布と実測値の分布を比較しても大きな乖離はない。   
```{r, fig.height = 4.5}
## 全期間    
pp_check(m_cohesion_es, ndraws = 100)+
  theme_bw()+
  theme(aspect.ratio = 0.9)

## 21年より前
pp_check(m_cohesion_es_bf21, ndraws = 100)+
  theme_bw()+
  theme(aspect.ratio = 0.9)
```  
<br/>  
...
Rhatにも問題はなく、収束の問題はないよう。  
```{r, fig.dim = c(8,4)}
data.frame(Rhat = brms::rhat(m_cohesion_es)) %>% 
  ggplot(aes(x = Rhat))+
  geom_histogram(fill = "white",
                 color = "black")+
  theme_bw()+
...
data.frame(Rhat = brms::rhat(m_cohesion_es_bf21)) %>% 
  ggplot(aes(x = Rhat))+
  geom_histogram(fill = "white",
                 color = "black")+
  theme_bw()+
...
まずは、全期間のデータを用いたモデルについてみる。太字になっている変数は95%確信区間が0をまたいでおらず、有意な影響があるといえる。  
```{r, echo = FALSE}
summary_cohesion_es <- summary(m_cohesion_es)

model_parameters(m_cohesion_es,  ci = 0.95,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,3,5,6, 7,8) %>% 
  mutate("Bulk ESS" = summary_cohesion_es$fixed$Bulk_ESS,
         "Tail ESS" = summary_cohesion_es$fixed$Tail_ESS) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),", ",sprintf("%.2f",CI_high),"]")) %>% 
  dplyr::select(c(1,2,3, 9), everything()) %>% 
  dplyr::select(-CI_low, -CI_high) %>% 
  mutate(pd = str_c(sprintf("%.2f",pd*100))) %>% 
  ...
  mutate(Parameter = str_replace_all(Parameter, c("b_Intercept"="Intercept",
                                                  ...
                                                  "b_kin_std" = "No. of kin individuals on the day"))) %>% 
  rename("Explanatory variables" = "Parameter") %>% 
  mutate(across(where(is.numeric), ~sprintf("%.2f", .))) -> table_cohesion_es

table_cohesion_es

write_csv(table_cohesion_es, "tables/table_cohesion_es.csv")
```
<br/>  

続いて、2021年のデータを除いたモデルについて確認する。  
```{r, echo = FALSE}
summary_cohesion_es_bf21 <- summary(m_cohesion_es_bf21)

model_parameters(m_cohesion_es_bf21,  ci = 0.95,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,3,5,6, 7,8) %>% 
  mutate("Bulk ESS" = summary_cohesion_es_bf21$fixed$Bulk_ESS,
         "Tail ESS" = summary_cohesion_es_bf21$fixed$Tail_ESS) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),", ",sprintf("%.2f",CI_high),"]")) %>% 
  dplyr::select(c(1,2,3, 9), everything()) %>% 
  dplyr::select(-CI_low, -CI_high) %>% 
  mutate(pd = str_c(sprintf("%.2f",pd*100))) %>% 
  ...
  mutate(Parameter = str_replace_all(Parameter, c("b_Intercept"="Intercept",
                                                  ...
                                                  "b_kin_std" = "No. of kin individuals on the day"))) %>% 
  rename("Explanatory variables" = "Parameter") %>% 
  mutate(across(where(is.numeric), ~sprintf("%.2f", .))) -> table_cohesion_es_bf21

table_cohesion_es_bf21

write_csv(table_cohesion_es_bf21, "tables/table_cohesion_es_bf21.csv")
```
<br/>  
...
まず、TYの有無についてです。  
```{r}
emm_cohesion_es_TY <- emmeans(m_cohesion_es,
                              ~TY_presence)
...
cohesion_data_es %>% 
  ...
  geom_boxplot(size = 0.5,
               ...
               fill = "grey",
               outlier.alpha = 0) +
  geom_point(data = emm_cohesion_es_TY %>% 
               data.frame() %>% 
               mutate(TY_presence = as.factor(TY_presence)),
             aes(y = emmean),
             size = 3,
             color = "white") +
  geom_errorbar(data = emm_cohesion_es_TY %>% 
               data.frame() %>% 
               mutate(TY_presence = as.factor(TY_presence)),
             aes(y = emmean, ymin = lower.HPD,
                 ymax = upper.HPD),
  ...
  theme(text = element_text(family = "Times New Roman"),
        axis.text.x = element_markdown(),
        aspect.ratio = 1) -> p_cohesion_es_TY

p_cohesion_es_TY

ggsave("figures/p_cohesion_es_TY.tif", p_cohesion_es_TY,
       dpi = 2000, units = "mm",
       width = 100, height = 100)
```
<br/>  
...
続いて、ITについてです。  
```{r}
emm_cohesion_es_IT <- emmeans(m_cohesion_es_bf21,
                              ~IT_presence)

cohesion_data_es_bf21 %>% 
  ggplot(aes(x = IT_presence, y = log(mean_RG))) +
  geom_violin(bw = 0.2,
              scale = "width") +
  geom_boxplot(size = 0.5,
               ...
               fill = "grey",
               outlier.alpha = 0) +
  geom_point(data = emm_cohesion_es_IT %>% 
               data.frame() %>% 
               mutate(IT_presence = as.factor(IT_presence)),
             aes(y = emmean),
             size = 3,
             color = "white") +
  geom_errorbar(data = emm_cohesion_es_IT %>% 
               data.frame() %>% 
               mutate(IT_presence = as.factor(IT_presence)),
             aes(y = emmean, ymin = lower.HPD,
                 ymax = upper.HPD),
  ...
  theme(text = element_text(family = "Times New Roman"),
        axis.text.x = element_markdown(),
        aspect.ratio = 1) -> p_cohesion_es_IT

p_cohesion_es_IT

ggsave("figures/p_cohesion_es_IT.tif", p_cohesion_es_IT,
       dpi = 2000, units = "mm",
       width = 100, height = 100)
```
<br/>  
