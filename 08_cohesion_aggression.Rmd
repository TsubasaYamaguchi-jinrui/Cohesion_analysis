...
model_parameters(m_agg_prox_an,  ci = 0.95, dispersion = T,
...
model_parameters(m_agg_prox_an_bf21,  ci = 0.95, dispersion = T,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,2, 3, 5, 6, 7,8) %>% 
  mutate("Bulk ESS" = summary_agg_prox_an_bf21$fixed$Bulk_ESS,
         "Tail ESS" = summary_agg_prox_an_bf21$fixed$Tail_ESS) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),", ",sprintf("%.2f",CI_high),"]")) %>% 
  dplyr::select(c(1,2,3, 10), everything()) %>% 
  dplyr::select(-CI_low, -CI_high) %>% 
  mutate(pd = str_c(sprintf("%.2f",pd*100))) %>% 
...
model_parameters(m_agg_prox_es,  ci = 0.95, dispersion = T,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,2, 3, 5, 6, 7,8) %>% 
...
model_parameters(m_agg_prox_es_bf21,  ci = 0.95, dispersion = T,
                 centrality = "median", ci_method = "eti") %>% 
...
model_parameters(m_agg_RG_an,  ci = 0.95, dispersion = TRUE,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,2, 3,  5,6, 7,8) %>% 
...
model_parameters(m_agg_RG_an_bf21,  ci = 0.95, dispersion = TRUE,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,2, 3, 5,6, 7,8) %>% 
...
## 自己相関の図示  
acf(focal_agg_RG_es_full2$resid, lag.max = 10, na.action = na.pass)

## Box-Ljung test
Box.test(focal_agg_RG_es_full2$resid, type = "Ljung-Box", lag = 10)
```
<br/>  

2021年交尾期を除いた場合はわずかにあるよう。    
```{r}
## 全時間ポイントの作成  
...
## 自己相関の図示  
acf(focal_agg_RG_es_bf21_full2$resid, lag.max = 10, na.action = na.pass)

## Box-Ljung test
Box.test(focal_agg_RG_es_bf21_full2$resid, type = "Ljung-Box", lag = 10)
```
<br/>  
...
model_parameters(m_agg_RG_es,  ci = 0.95, dispersion = TRUE,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "sigma") %>% 
  data.frame() %>%
  dplyr::select(1,2, 3,  5,6, 7,8) %>% 
...
model_parameters(m_agg_RG_es_bf21,  ci = 0.95, dispersion = TRUE,
                 centrality = "median", ci_method = "eti") %>% 
  filter(Parameter != "phi") %>% 
  data.frame() %>%
  dplyr::select(1,2,3, 5,6, 7,8) %>% 
  mutate("Bulk ESS" = summary_agg_RG_es_bf21$fixed$Bulk_ESS,
         "Tail ESS" = summary_agg_RG_es_bf21$fixed$Tail_ESS) %>% 
  mutate("95%CI" = str_c("[",sprintf("%.2f",CI_low),", ",sprintf("%.2f",CI_high),"]")) %>% 
  dplyr::select(c(1,2,3, 10), everything()) %>% 
  dplyr::select(-CI_low, -CI_high) %>% 
  mutate(pd = str_c(sprintf("%.2f",pd*100))) %>% 
