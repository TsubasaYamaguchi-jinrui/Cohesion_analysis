...
#### モデリング  
### モデリング  
それでは、モデリングを行います。分析には、先ほどと同様に240分以上群れを追跡した日のデータのみを用います。    
...
predict_agginj <- predict_response(m_agginj,
                                   terms = c("rate_agg_std[-0.42:16.25, by = 0.1]", "rs2[0:1, by = 1]"),
                                   type = "fixed",
...
injury_analysis %>% 
  ...
  labs(x = "Frequency of aggression (times/h)", y = "Presence of new injury",
